...
'''
Calculate catchment-scale NDVI/LAI time-series based on MODIS.
...
def summary_year(year, data_root, out_dir, root_dir):
    ...
    for key in res:
        ...
        df = df.rename(columns={0: 'lai'})
        df.to_excel(os.path.join(out_dir, str(year), f'{key}.xlsx'))
        final_res[key] = [df['lai'].max(), df['lai'].min(), df['lai'].mean()]
        return final_res
...
if __name__ == '__main__':
    ...
    for year in range(2002, 2003):
        if not os.path.isdir(f'./output/lai/{year}'):
            os.mkdir(f'./output/lai/{year}')
        tmp_res = summary_year(year, data_root='./data/MCD15A3H', out_dir='./output/lai', root_dir='./')
        for key in tmp_res:
            if not key in res:
                res[key] = []
